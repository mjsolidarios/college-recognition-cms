  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[95vw] max-w-xl -translate-x-1/2 -translate-y-1/2 rounded-xl border border-[var(--color-hairline)] bg-white p-5 animate-scale-in">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div>
              <Dialog.Title className="text-sm font-semibold text-[var(--color-ink)]">Export Booklet Data</Dialog.Title>
            ...
            <Dialog.Close asChild>
              <Button type="button" variant="ghost" size="icon" className="size-7 flex-shrink-0 text-[var(--color-muted)]"><X className="size-4" /></Button>
            </Dialog.Close>
          </div>
    ...
    <Dialog.Root open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset() }}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[95vw] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-xl border border-[var(--color-hairline)] bg-white p-5 animate-scale-in">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div>
              <Dialog.Title className="text-sm font-semibold text-[var(--color-ink)]">Import Booklet Data</Dialog.Title>
              <Dialog.Description className="mt-0.5 text-xs text-[var(--color-muted)]">Upload a .json file or paste JSON content</Dialog.Description>
            </div>
            <Dialog.Close asChild>
              <Button type="button" variant="ghost" size="icon" className="size-7 flex-shrink-0 text-[var(--color-muted)]"><X className="size-4" /></Button>
            </Dialog.Close>
              ...
              {state.step === 'valid' && (
                <div className="animate-fade-in rounded-lg border border-emerald-200 bg-emerald-50 p-3 space-y-2">
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <Button type="button" size="sm" onClick={handleConfirm} className="border-[#1f8a65] bg-[#1f8a65] text-white hover:border-[#1a7456] hover:bg-[#1a7456]">
